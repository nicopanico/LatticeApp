...
tic
PatientID = '141-10';
radius = 7.5;                            % mm, raggio sfere (Duriseti 7.5)
...
[indOpt, integ_ovh, ovhf] = optimize_OVH4(path, CTinfo, zDim, nOAR, ind, to_evaluate, vertices, radius, vq, strList,teta,Vrot, PatientID);
fileName = sprintf('%s%s%s', PatientID, '_', 'OVH Data');
sava(fileName, 'integ_ovh', 'ovhf');
...
figName = springtf('%s%s%s', PatientID, '_', 'contourOut', '_', 'best.png');
saveas(gcf, figName)
